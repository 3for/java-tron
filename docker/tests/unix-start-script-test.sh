...
template = template.replace("${classpath}", "$APP_HOME/lib/java-tron.jar")
template = re.sub(
...
expected = sys.argv[2:]
try:
    classpath_index = decoded.index("-classpath")
except ValueError:
    raise SystemExit("Java argv did not include -classpath:\n" + "\n".join(decoded))
expected_classpath = str(
    (pathlib.Path(sys.argv[1]).parent / "java-tron/lib/java-tron.jar").resolve()
)
if decoded[classpath_index + 1] != expected_classpath:
    raise SystemExit(
        "Java classpath did not resolve the distribution library.\nExpected:\n  "
        + expected_classpath
        + "\nActual:\n  "
        + decoded[classpath_index + 1]
    )
if actual != expected:
    raise SystemExit(
