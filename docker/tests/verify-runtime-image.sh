...
docker run --rm --entrypoint sh "$image" -ec '
  ...
  if touch /java-tron/.write-test 2>/dev/null; then
    echo "The runtime user unexpectedly wrote to /java-tron" >&2
    exit 1
  fi
...
docker run --rm --security-opt no-new-privileges --entrypoint sh \
  ...
  "$image" -ec '
    ...
    test -f /java-tron/output-directory/.write-test
    test -f /java-tron/logs/.write-test
  '
