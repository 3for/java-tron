...
assert_job_not_contains() {
  ...
  if awk -v job="$job" -v unexpected="$unexpected" '
    $0 == "  " job ":" { in_job = 1; next }
    in_job && /^  [^[:space:]]/ { exit }
    in_job && index($0, unexpected) { found = 1 }
    END { exit !found }
  ' "$workflow"; then
    echo "Docker CI job $job unexpectedly contains: $unexpected" >&2
    exit 1
  fi
}
...
assert_job_not_contains remote-build-amd64 'run: bash docker/tests/vmoptions-test.sh "$IMAGE"'
...
assert_job_not_contains remote-build-arm64 'run: bash docker/tests/vmoptions-test.sh "$IMAGE"'
